...
# Function to classify policy information
def classify_policy_info(policy_info, model):
    prompt = f"Classify the policy with the following details: " \
             f"Policy Number: {policy_info.get('Policy Number')}, " \
             f"Policy Gross Premium: {policy_info.get('Policy Gross Premium')}, " \
             f"Policy Period Start: {policy_info.get('Policy Period (Start)')}, " \
             f"Policy Period End: {policy_info.get('Policy Period (End)')}, " \
             f"Sum Insured: {policy_info.get('Sum Insured')}, " \
             f"Key Coverages & Benefits: {policy_info.get('Key Coverages & Benefits')}."
    try:
        response = model.generate_content(prompt)
        classification = response.text.strip()
        print(f"Classification: {classification}")  # Debug statement
        return classification
    except Exception as e:
        print(f"Error during classification: {e}")
        return None

# Function to rate policy information
def rate_policy_info(policy_info, model):
    prompt = f"Rate the policy with the following details: " \
             f"Policy Number: {policy_info.get('Policy Number')}, " \
             f"Policy Gross Premium: {policy_info.get('Policy Gross Premium')}, " \
             f"Policy Period Start: {policy_info.get('Policy Period (Start)')}, " \
             f"Policy Period End: {policy_info.get('Policy Period (End)')}, " \
    ...
    try:
        ...
        print(f"Rating: {rating}")  # Debug statement
        return rating
    except Exception as e:
...
def process_uploaded_pdfs(uploaded_files):
    ...
    for uploaded_file in uploaded_files:
        ...
        # Classify policy information
        policy_info["Classification"] = classify_policy_info(policy_info, model)

        # Rate policy information
        policy_info["Rating"] = rate_policy_info(policy_info, model)

        policy_data.append(policy_info)

    # Prepare data for CSV (only filename, key coverages, classification, and rating)
    policy_data_csv = [{'Filename': data['Filename'], 'Key Coverages & Benefits': data['Key Coverages & Benefits'], 'Classification': data['Classification'], 'Rating': data['Rating']} for data in policy_data]
    
    # Save the extracted information to a CSV file
    if os.path.exists("policy_data_1.csv"):
        existing_policy_df = pd.read_csv("policy_data_1.csv")
    ...
    policy_df_csv.to_csv("policy_data_1.csv", index=False, columns=['Filename', 'Key Coverages & Benefits', 'Classification', 'Rating'], header=True)
    print("Policy information saved to policy_data_1.csv.")
    ...
    if os.path.exists("policy_data_1.xlsx"):
        existing_excel_df = pd.read_excel("policy_data_1.xlsx")
        excel_df = pd.concat([existing_excel_df, pd.DataFrame(excel_data)], ignore_index=True)
    else:
        excel_df = pd.DataFrame(excel_data)
    
    # Save summary information to Excel
    excel_df.to_excel("policy_data_1.xlsx", index=False, header=True)
    print("Summary information saved to policy_data_1.xlsx.")
...
def main():
    ...
    if uploaded_files:
        ...
        st.markdown(get_download_link("policy_data_1.csv", "Download policy_data_1.csv"), unsafe_allow_html=True)
        st.markdown(get_download_link("policy_data_1.xlsx", "Download policy_data_1.xlsx"), unsafe_allow_html=True)
